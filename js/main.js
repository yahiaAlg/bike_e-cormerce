...
(function(){
  ...
  var MEDIA = {
    ...
    'motorcycle riding forest road trail wide': 'photo-1435244837924-21c508f9db25', // Gabriel Sanchez
  ...
  function placeholderSVG(w,h){
    var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="'+w+'" height="'+h+'" viewBox="0 0 '+w+' '+h+'">'
      ...
      + '<path d="M0,'+(h*0.7)+' Q '+(w*0.3)+','+(h*0.5)+' '+w+','+(h*0.65)+'"/>'
      + '<path d="M0,'+(h*0.82)+' Q '+(w*0.4)+','+(h*0.6)+' '+w+','+(h*0.8)+'"/>'
      + '</g>'
      + '<circle cx="'+(w/2)+'" cy="'+(h/2)+'" r="3" fill="#F2E900"/>'
      + '</svg>';
  ...
  function resolve(img){
    ...
    if(!q) return;
    var parts = q.split(',');
    var query = parts[0], w = parts[1] || 1200, h = parts[2] || 900;
    var id = MEDIA[query];
    img.src = id
      ? 'https://images.unsplash.com/' + id + '?auto=format&fit=crop&q=80&w=' + w + '&h=' + h
      : placeholderSVG(w, h);
    img.loading = img.loading || 'lazy';
    img.onerror = function(){ img.onerror=null; img.src = placeholderSVG(w,h); };
  }
...
(function(){
  ...
  tl.to('.hero-title .line span', {yPercent:0, duration:1.1, stagger:.12})
    .to('.hero-kicker', {opacity:1, y:0, duration:.7}, '-=.9')
    .to('.hero-sub', {opacity:1, y:0, duration:.7}, '-=.7')
    .to('.hero-actions', {opacity:1, y:0, duration:.7}, '-=.6')
    .to('.hero-media img', {scale:1, duration:2.4, ease:'power2.out'}, 0);

  gsap.set(['.hero-kicker','.hero-sub','.hero-actions'], {opacity:0, y:20});
  ...
  if(batterySection){
    var bTl = gsap.timeline({
      scrollTrigger:{
        trigger: batterySection, start:'top top', end:'+=140%',
        scrub:1, pin:true
      }
    });
    bTl.to('#batteryOld', {yPercent:0, top:'56%', y:'-160%', opacity:1, duration:.3})
       .to('#batteryOld', {y:'40%', opacity:.2, duration:.35})
       .to('#batteryGlow', {opacity:1, duration:.2}, '-=.15')
       .to('#batteryNew', {y:'0%', top:'56%', opacity:1, duration:.35}, '-=.1')
       .to('#batteryGlow', {opacity:0, duration:.3})
       .to('#batteryProgress', {width:'100%', duration:1}, 0);
  }

  /* ---- Cinematic image micro-scale on enter ---- */
  gsap.utils.toArray('.cine-media img, .final-media img').forEach(function(img){
  ...
  if(form){
    form.addEventListener('submit', function(e){
      ...
      setTimeout(function(){ icon.className = 'bx bx-right-arrow-alt'; }, 2200);
    });
  }
  ...
  document.querySelectorAll('a[href^="#"]').forEach(function(a){
    a.addEventListener('click', function(e){
      ...
      if(target){
        e.preventDefault();
        gsap.to(window, {duration:1.1, ease:'power3.inOut', scrollTo:{y:target, offsetY:70}});
      }
    });
  });
})();
...
(function(){
  ...
  gsap.matchMedia().add('(min-width: 861px)', function(){
    ...
    var tween = gsap.to(track, {
      ...
      ease:'none',
      scrollTrigger:{
        trigger: pin, start:'top top', end: function(){ return '+=' + (getDistance()*1.1); },
...
(function(){
  ...
  var stops = [
    {label:'Start', text:"Battery charged, tyres checked. The trail hasn't been ridden yet — that's the best part."},
    {label:'Forest', text:'Tight single-track and low light. Instant torque makes the tricky lines feel simple.'},
    {label:'Valley', text:'Open ground, longer sightlines. Time to open the throttle and let the RVX stretch out.'},
    {label:'Ridge', text:'Loose rock and elevation. Long-travel suspension keeps both wheels talking to the ground.'},
    {label:'Summit', text:'Silence, and a view that was worth every switchback. Swap the battery, ride back down.'}
  ];
  var pointPositions = [];
  points.forEach(function(g){
    var c = g.querySelector('circle');
    pointPositions.push({x:parseFloat(c.getAttribute('cx')), y:parseFloat(c.getAttribute('cy')), el:g});
  });
  ...
  function nearestStopIndex(progress){
    ...
    pointPositions.forEach(function(p, i){
      ...
      var samplePt = path.getPointAtLength((i/(pointPositions.length-1)) * len);
      var diff = Math.abs(samplePt.x - p.x) + Math.abs(samplePt.y - p.y);
    });
    return Math.min(stops.length-1, Math.round(progress*(stops.length-1)));
  }

  ScrollTrigger.create({
    trigger: pin, start:'top top', end:'+=220%',
    scrub:1, pin:true, invalidateOnRefresh:true,
    onUpdate: function(self){
      ...
      pointPositions.forEach(function(p, i){
        var active = i <= Math.floor(progress*(pointPositions.length-1)+0.001);
        p.el.querySelector('.route-point').classList.toggle('active', i === idx || active && i===idx);
...
(function(){
  ...
  function goTo(i){
    ...
    var curSlide = slides[index], nextSlide = slides[next];
    gsap.timeline()
      .to(curSlide.querySelector('img'), {scale:1.06, duration:.5, ease:'power2.in'}, 0)
      .set(curSlide, {className:'c-slide'})
      .set(nextSlide, {className:'c-slide is-active'})
      .fromTo(nextSlide.querySelector('img'), {scale:1.12, opacity:0}, {scale:1, opacity:1, duration:.7, ease:'power3.out'}, 0)
      .fromTo(nextSlide, {opacity:0}, {opacity:1, duration:.4}, 0);
    index = next;
